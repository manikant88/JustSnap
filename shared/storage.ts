import type { ActivityEvent, Capture, CaptureGroup, LibraryState, PendingUsage } from "./types";
...
export async function getLibrary(): Promise<LibraryState> {
  const result = await chrome.storage.local.get([CAPTURES_KEY, GROUPS_KEY, EVENTS_KEY, PENDING_KEY]);
  return {
    captures: (result[CAPTURES_KEY] as Capture[] | undefined) ?? [],
    groups: (result[GROUPS_KEY] as CaptureGroup[] | undefined) ?? [],
...
export async function saveLibrary(library: Pick<LibraryState, "captures" | "groups" | "events">): Promise<void> {
